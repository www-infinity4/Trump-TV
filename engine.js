(function(root){
  ...
  // viewer resolves that format against one shared U.S. broadcast clock.
  ...
  function createDaySchedule(nowMs,catalog,blocks){
    const p=parts(new Date(nowMs));const midnightMs=zonedToUtc(p.year,p.month,p.day);const key=dateKey(nowMs);const epochDay=Math.floor(midnightMs/86400000);const weekNumber=Math.floor(epochDay/7);const schedule=[];
    for(const block of blocks){
      const pool=(catalog[block.pool]||[]).filter(item=>item.videoId||item.sourceUrl);if(!pool.length)continue;
      const base=rotate(pool,`infinity-program-week-v2:${weekNumber}:${block.id}`);const estimatedSlots=Math.max(1,(block.endHour-block.startHour)*2);const offset=((epochDay*estimatedSlots)%base.length+base.length)%base.length;const ordered=[...base.slice(offset),...base.slice(0,offset)];const startSec=block.startHour*3600;const endSec=block.endHour*3600;
